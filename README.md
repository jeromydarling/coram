...
**Live at https://coram.jer-f84.workers.dev** — Postgres 17 on Neon behind
Hyperdrive, all ten migrations applied, signup through export exercised against
it, and tenant isolation verified between two real workspaces.
...
Every module exists in code and the schema is deployed, but only Foundation and
Membra have actually been exercised end to end — signup, login, contacts,
export, and cross-tenant isolation. The other nine modules have tables and
policies on a live database and no traffic through them yet. `/trust` states
plainly that no artifact has been published, because none has.

The §8.2 photography is generated by `npm run imagery:generate` (FLUX.2 on
Workers AI) and needs Cloudflare credentials; until it runs, `<Picture>` falls
back to a tone block and the site is complete without it.

Four bugs only the live stack could find, all now fixed and covered:
Workers caps PBKDF2 at 100k iterations so the 600k KDF threw outright; every
database route called `close(sql)` eagerly and tore down its own connection;
`0002` created a policy that forward-referenced a function; and static assets
matched `/` before the Worker, hiding the marketing site in production only.
